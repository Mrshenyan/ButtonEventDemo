...
import { CallbackObject } from "./NetInterface";
...
export default class NetManager {
    ...
    setNetNode(newNode:NetNode,channelID:number=0){
   ...
   public requestUnique(buf: NetData, rspCmd: number, rspObject, showTips: boolean = true, force: boolean = false, channelId: number = 0): boolean {
    let node = this._channels[channelId];
    if(node) {
